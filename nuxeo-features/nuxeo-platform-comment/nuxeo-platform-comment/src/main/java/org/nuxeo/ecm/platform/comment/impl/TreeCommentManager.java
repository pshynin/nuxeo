...
public class TreeCommentManager extends AbstractCommentManager {
    ...
    public Comment createComment(CoreSession s, Comment comment) {
        ...
        return CoreInstance.doPrivileged(s, session -> {
            ...
            handleNotificationAutoSubscriptions(session, commentDocModel, topLevelDocument);
    ...
    protected void handleNotificationAutoSubscriptions(CoreSession session, DocumentModel commentDocModel,
        ...
        if (!hasComments(session, document)) {
            ...
            subscribeToNotifications(document, topLevelDocumentAuthor);
        }
        ...
        if (commentAuthor != null && !hasComments(session, document, commentAuthor.getName())) {
            // Comment author is writing his first comment on the document
            subscribeToNotifications(document, commentAuthor);
        }
    }
