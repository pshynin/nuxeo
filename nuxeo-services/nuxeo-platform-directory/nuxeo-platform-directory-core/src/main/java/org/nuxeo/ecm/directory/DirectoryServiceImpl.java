...
public class DirectoryServiceImpl extends DefaultComponent implements DirectoryService {
    ...
    @Override
    public void loadFromCSV(String directoryName, Blob dataFile, String dataLoadingPolicy) {
        if (StringUtils.isBlank(dataLoadingPolicy) || dataLoadingPolicy.equals(NEVER_LOAD)) {
            throw new DirectoryException("bad dataLoadingPolicy : " + dataLoadingPolicy , SC_BAD_REQUEST);
        }
        Directory directory = getDirectoryOrFail(directoryName);
        directory.loadFromCSV(dataFile, dataLoadingPolicy);
    }
