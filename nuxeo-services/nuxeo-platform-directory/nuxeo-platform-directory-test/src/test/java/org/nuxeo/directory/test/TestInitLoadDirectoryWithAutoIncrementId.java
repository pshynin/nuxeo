...
public class TestInitLoadDirectoryWithAutoIncrementId {
    ...
    protected void assertInitialState() {
        try (Session session = directoryService.open(CSV_LOAD_DIRECTORY)) {
            assertEquals(3, TestInitLoadDirectory.queryFullDirectory(session).size());
            assertEquals("European Union", TestInitLoadDirectory.queryEurope(session).getPropertyValue("label"));
        }
    }
}
